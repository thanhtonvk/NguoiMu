...
nhandientien::nhandientien() {
    blob_pool_allocator.set_size_compare_ratio(0.f);
    workspace_pool_allocator.set_size_compare_ratio(0.f);
}

int nhandientien::load(AAssetManager *mgr, int _target_size, const float *_norm_vals, bool use_gpu) {
    yolo.clear();
...
int nhandientien::detect(const cv::Mat &rgb, std::vector<Object> &objects, float prob_threshold,
                         float nms_threshold) {
    int width = rgb.cols;
    int height = rgb.rows;
...
int nhandientien::draw(cv::Mat &rgb, const std::vector<Object> &objects) {
