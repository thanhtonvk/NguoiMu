...
int yolov11::load(AAssetManager *mgr, int _target_size, const float *_norm_vals, bool use_gpu) {
    yolo.clear();
...
int yolov11::detect(const cv::Mat &rgb, std::vector<Object> &objects, float prob_threshold,
                   float nms_threshold) {
    int width = rgb.cols;
    int height = rgb.rows;
