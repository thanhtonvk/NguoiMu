...
indoor_detection::indoor_detection() {
    blob_pool_allocator.set_size_compare_ratio(0.f);
    workspace_pool_allocator.set_size_compare_ratio(0.f);
}

int indoor_detection::load(AAssetManager *mgr, int _target_size, const float *_norm_vals,
                           bool use_gpu) {
    yolo.clear();
...
//#endif
    ...
    char parampath[256];
    char modelpath[256];
    sprintf(parampath, "indoor.param");
    sprintf(modelpath, "indoor.bin");

    yolo.load_param(mgr, parampath);
    yolo.load_model(mgr, modelpath);

    __android_log_print(ANDROID_LOG_DEBUG, "LOGFACE", "load model ok");
...
int indoor_detection::detect(const cv::Mat &rgb, std::vector<Object> &objects, float prob_threshold,
                             float nms_threshold) {
    int width = rgb.cols;
    int height = rgb.rows;
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOGFACE", "load model ok %zu", objects.size());
...
int indoor_detection::draw(cv::Mat &rgb, const std::vector<Object> &objects) {
    static const char *class_names[] = {"cua", "cau thang"};
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOGFACE", "detect");
