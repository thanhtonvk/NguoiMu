...
public class CauHoiActivity extends AppCompatActivity {
    ...
    private void handleAnswer(String selectedAnswer) {
        Common.dapanChon.add(selectedAnswer);
        if (Common.CAU_HOI != null && checkAnswer(selectedAnswer)) {
            dung++;
            delayAndProceed(this::nextQuestion, 1000);
        } else {
            sai++;
            delayAndProceed(this::nextQuestion, 1000);
        }
    ...
    private void nextQuestion() {
        if (++idxHocTap >= Common.cauHoiArrayList.size()) {
            ...
            Common.cauHoiDaTraLoi.add(Common.CAU_HOI);
            updateUIForQuestion();
        }
    }
    ...
    private void showResultDialog() {
        ...
        String noiDung = "Bạn đã hoàn thành bài kiểm tra. Trả lời đúng " + dung + " câu hỏi. "
        ...
        textToSpeech.setOnUtteranceProgressListener(new UtteranceProgressListener() {
            ...
            public void onError(String utteranceId) {
                runOnUiThread(() -> Toast.makeText(CauHoiActivity.this, "Lỗi khi phát âm", Toast.LENGTH_SHORT).show());
            }
        });
